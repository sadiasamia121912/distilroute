...
Teacher (`gpt-oss-120b`, zero-shot): accuracy **0.892** on the 120 test queries labelled so far (4% — intent-sorted, so partial coverage is not a random sample). Agreement below is measured on those rows.
