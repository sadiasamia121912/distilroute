...
**Résumé bullet target:** "Distilled a 70B-parameter LLM ticket router into a 66M-parameter
...
- **Teacher is zero-shot**: intent names only, no examples. Giving it examples would leak gold
  labels into the "LLM-labelled" data and muddy the story.
...
Primary **Groq** (`llama-3.3-70b-versatile`), fallback **Google AI Studio** (Gemini Flash).
...
- [ ] **1.4** Get a Groq key (free, https://console.groq.com) → `.env` as `GROQ_API_KEY`. _Your click._
